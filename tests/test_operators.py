...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if not math.isclose(a, b, abs_tol=1e-9) and abs(max(a, b)) < 6:
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    or math.isclose(b, c, abs_tol=1e-9) \
    or math.isclose(a, c, abs_tol=1e-9):
        return
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    msum = minitorch.operators.sum
    assert_close(msum([msum(ls1), msum(ls2)]), msum(addLists(ls1, ls2)))
